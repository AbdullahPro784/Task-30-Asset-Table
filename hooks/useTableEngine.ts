...
export function useTableEngine(initialData: Asset[]) {
    ...
    // State
    const [data, setData] = usePersistedData(initialData, "original-table-data");
        ...
        if (typeof window !== "undefined") {
            ...
            if (savedOrder) {
                try {
                    ...
                    if (!parsedOrder.includes("drag-handle")) {
                        parsedOrder.unshift("drag-handle");
                    }
    ...
    const {
        ...
        activeId,
        activeItem
    } = useTableDragDrop(data, setData, setColumnOrder, 'flat');
